...
> коде, отмечены как «требует уточнения» — см. также раздел
> [Пробелы в документации](#пробелы-в-документации) внизу.
...
В репозитории нет сохранённых скриншотов UI. Ниже — плейсхолдеры под
основные экраны (заменить реальными снимками):

| Экран | Маршрут | Заглушка |
| --- | --- | --- |
| Дашборд | `/` | `[ дашборд: проблемные сплиты · причины ошибок · позиции в протоколах ]` |
| Плеер тренировки | `/trainings/{id}/play` | `[ карта + трек + слайдер воспроизведения ]` |
| Протокол соревнования | `/race-results/{id}` | `[ таблица сплитов всех участников + отставание от лидера ]` |
| Разбор сплита с AI-тренером | модальное окно на дашборде/протоколе | `[ карта сплита + чат с тренером справа ]` |
...
См. итоговый раздел в конце этого чата/задачи — краткий список того, что
осталось неясным из кода, и какие места стоило бы документировать (или
комментировать в самом коде) подробнее.
